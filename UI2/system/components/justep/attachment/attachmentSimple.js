	...
	var AttachmentSimple = BindComponent.extend({
		...
		initUploader:function(){
			...
			self.uploader.on('onStart',function(event){
				...
				var fileExtension = event.file.name.substring(event.file.name.lastIndexOf('.'));
				var storeName = (new justep.UUID()).valueOf();
				var storeFileName = storeName + fileExtension;
				event.file.data = { 
        ...
        */ 
    	...
    	removeItemByStoreID:function(storeFileName,ownerID){
    		...
    		this.bindData.setValueByID(this.bindRelation,JSON.stringify(items),this.bindData.getRowID(this.ref['define'].row));
    	},
    	...
    	getOwnerID : function($object){
    		...
    		return document.getElementsByClassName('userID')[0].value;
    	} ,
    	getItems : function(ownerID){
    		...
    		value = this.bindData.getValueByID(this.bindRelation, this.bindData.getRowID(this.ref['define'].row));
    		if(value){
    			try {
