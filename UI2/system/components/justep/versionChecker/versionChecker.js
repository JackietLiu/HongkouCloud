define(function(require) {
	...
	VersionChecker.prototype.check = function(i) {
	...
	VersionChecker.prototype.checkAppVersion = function(i) {
		var self = this;
		if (window.cordova && cordova.getAppVersion) {
			cordova.getAppVersion
					.getVersionNumber(function(currentVersion) {
						if (self.appInfo[self.env] && self.versionCompare(currentVersion,self.appInfo[self.env].version)
								&& window.localStorage && localStorage.getItem('versionIgnore') !== self.appInfo[self.env].version) {
							self.versionMsg.show({
								type : "YesNoCancel",
								title : "版本更新",
								message : self.appInfo[self.env].changeLog
							});
						}else if(i===0 && self.appInfo[self.env] && currentVersion <self.appInfo[self.env].version){
							self.versionMsg.show({
								type : "YesNoCancel",
								title : "版本更新",
								message : self.appInfo[self.env].changeLog
