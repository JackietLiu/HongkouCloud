define(function(require) {
	...
	var _getMsgContent = function(msg, option,isBot) {
		option = option || {};
		...
		if (type === 'service' && msg.content.text === msg_createPersonChatDialog) {
			...
			if(isBot){
				var msgOption = new Function("","return"+msg.content.text)();//eval("(" + msg.content.text + ")");
				content = justep.String.format("<a class='{0}'><div>{1}</div><div style = 'text-align:right;'>{2}</div></a>", option.botClass || '', msgOption.text,"------查看");
			}else{				
			...
			content = justep.String
					.format(
							"<audio class='hide' onerror = 'justep.Util.callModelFn({fn:\"updateVocText\",domNode:this},this,true);' oncanplay='justep.Util.callModelFn({fn:\"updateVocText\",domNode:this},this);'><source src='{0}'/><source src='{1}'/><source src='{2}'/></audio><i class='icon-loading-b {3}'/><label></label>",
	...
	var IMImpl = Object.extend({
			...
			require([ '../../lib/actor/actorClient' ], function(actorClient) {
				...
				ActorClient.promise.done(function(client) {
					...
					if (ActorClient.isLoggedIn(pid)) {
						...
						client.validateToken(pid, pname, token).then(function(state) {
							if (state === "logged_in") {
								loginOKdeferred.promise().then(function() {
									self.loadPerson(persons, pid).done(function() {
		...
		registerUsers : function(userIds, userNames) {
			var self = this;
			var deferred = $.Deferred();
			_api().registerUsers(userIds, userNames).then(function(result) {
				var rows = [];
				...
				self.updatePersonUid(null, null, rows).done(function() {
					deferred.resolve(result);
				});
			}, function(error) {
				deferred.reject(error);
		...
		messageParse : function(message, option) {
			if (message) {
				...
				var user = _api().getUser(message.sender.peer.id);
				var botMsgOption;
				if(user.isBot){
					botMsgOption = new Function("","return"+message.content.text)();//eval("(" + message.content.text + ")");
				}
				var attentionNum = 0;
				var isOwnSet;
				...
				return {
					mid : message.rid,
					content : _getMsgContent(message, option,user.isBot),
					sender : person,
					...
					pageUrl : botMsgOption?botMsgOption.url:'',
					pageParams : botMsgOption?botMsgOption.data:''
				};
			}
		},
		...
		validateToken : function(userId, userName, token) {
			return _api().validateToken(userId, userName, token);
		},
		searchText : function(peer, query, limit, offset) {
			return _api().searchText(peer, query, limit, offset);
		},
		searchFiles : function(peer, limit, offset) {
			return _api().searchFiles(peer, limit, offset);
		}
	});
