define(function(require) {
	...
	var Model = function() {
		...
		this.isBot = justep.Bind.observable(false);
	};
				...
				for (i = index + 1; i < messages.length; i++) {
					...
					if (message) {
						rows.unshift({
							...
							fSender : message.sender.name || message.senderName,
	...
	Model.prototype.modelModelConstructDone = function(event) {
		if (this.params) {
			...
			var initPeer = function(id) {
				...
				// 判断是否是机器人
				if (self.curPeer.type === 'user') {
					var user = IM.getUser(self.curPeer.id);
					if (user.isBot) {
						self.isBot.set(user.isBot);
					}
				}
				self.initClient();
			};
	...
	Model.prototype.initEvent = function() {
		...
		$(msgList).on("click.botMsg", "a.botMsg", this.botMsgClickHandle.bind(this));
	...
	Model.prototype.updateVocText = function(voc, error) {
		if (!error) {
			if (voc) {
				$(voc).prop('_isLoaded', true).next().removeClass('icon-loading-b').addClass('glyphicon glyphicon-volume-up').next('label').text(
						time2Str(voc.currentTime) + '/' + time2Str(voc.duration));
			}
		} else {
			if (voc) {
	...
	Model.prototype.playVoiceHandle = function(event) {
		...
		if (voc && voc._isLoaded)
			voc._isplaying ? voc.pause() : voc.play();
	};
	...
	Model.prototype.transmitBtnClick = function(event) {
		...
		// var text = mspPopMenu._fileUrl ? mspPopMenu._fileUrl :
		// mspPopMenu._fContent;
	...
	Model.prototype.optionTransmitBtnClick = function(event) {
		...
		this.comp("transmitWindowDialog").open({
			...
			// IM.sendTextMessage(option.peer, row.val("fileUrl") ?
			// row.val("fileUrl") : row.val("fMessageText"));
			// }
			// });
			// if (option.count === option.index) {
			// if (count === index) {
	...
	Model.prototype.transmitWindowDialogReceive = function(event) {
		var mspPopMenu = this.comp("msgPopMenu");
		...
		var data = this.comp('messageData');
		if (!this.more.get()) {
			$.each(event.data, function(i, v) {
				if (v.type === 'group') {
					IM.sendTextMessage(v.peer, text);
				} else {
					if (v.id !== IM.getCurrentPersonID()) {
						pids.push(v.id);
						pnames.push(v.name);
					}
				}
			...
			$.each(event.data, function(i, v) {
				if (v.type === 'group') {
					data.each(function(params) {
						row = params.row;
						if (row.val("fChecked") === 1 && row.val("fStyle") !== "audio") {
							IM.sendTextMessage(v.peer, row.val("fileUrl") ? row.val("fileUrl") : row.val("fMessageText"));
						}
					});
				} else {
					if (v.id !== IM.getCurrentPersonID()) {
			...
			IM.registerUsers(pids, pnames).done(function(result) {
				$.each(result, function(i, v) {
					peer = IM.getUserPeer(v.id);
					data.each(function(params) {
						row = params.row;
						if (row.val("fChecked") === 1 && row.val("fStyle") !== "audio") {
							IM.sendTextMessage(peer, row.val("fileUrl") ? row.val("fileUrl") : row.val("fMessageText"));
						}
					});
				});
			});
	...
	//bot发送消息
	Model.prototype.botMsgClickHandle = function(event){
		if (event.button === 0) {
			var ctx = justep.Bind.contextFor(event.target);
			var pageUrl = ctx.$object.val("pageUrl");
			var pageParams = ctx.$object.val("pageParams");
			if(pageUrl){				
				justep.Shell.showPage(pageUrl, {
					data : pageParams
				});
			}
		}
	...
	Model.prototype.msgListAfterRender = function(event) {
		function debounce(fn, wait, scope) {
			return function() {
				var args = arguments, later = function() {
