define(function(require) {
	...
	ActorClient.prototype.registerUsers = function(userIds, userNames) {
		return this.messager.registerUsers(userIds, userNames);
	};
	ActorClient.prototype.validateToken = function(userId, userName, token) {
		var result = $.Deferred();
		try {
			this.messager.validateToken(userId, userName, token, function(state) {
				result.resolve(state);
			}, function(error) {
				result.reject(error);
			});
	...
	ActorClient.prototype.searchText = function(peer, query, limit, offset) {
		return this.messager.findAllText(peer, query, limit, offset);
	};
	ActorClient.prototype.searchFiles = function(peer, limit, offset) {
		return this.messager.findAllFiles(peer, limit, offset);
	};
	return {
