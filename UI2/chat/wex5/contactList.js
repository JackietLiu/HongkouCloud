define(function(require) {
	...
	Model.prototype.loadAllData = function() {
		...
		IM.bindGroupDialogs(function(groups) {
			...
			if (groups.length > 0) {
				if(groups.length === 3){
					for ( i = 0; i < groups[1].shorts.length; i++) {
						 id = groups[1].shorts[i].peer.peer.id;
						 group = IM.getGroup(id);
						self.groupData.push({
							sID : id,
					...
					for ( i = 0; i < groups[0].shorts.length; i++) {
						 id = groups[0].shorts[i].peer.peer.id;
						 group = IM.getGroup(id);
						self.groupData.push({
							sID : id,
							sName : group.name + '(' + group.members.length + '人)',
	...
	Model.prototype.sendBtnClick = function(event) {
		if (!this.params.createGroup) {
			this.owner.send(this.transmit);
		} else {
			var rows = [];
	...
	Model.prototype.getDataArray = function(row) {
		...
		if (row.val("type") === 'group') {
			...
			index = "person_" + row.val("fSpersonID");
			if (!this.transmit[index] && row.val("fCheckRow") === "1") {
				this.transmit[index] = {
					id : row.val("fSpersonID"),
					name : row.val("fsName"),
					type : 'user'
				};
				this.transmitNum++;
